...
export default async function PublicacionesPage({ searchParams }: PageProps) {
  ...
  return (
    <main className="w-full bg-[#f5f7f5] min-h-screen">
      {/* 1. Hero Section */}
      <section className="w-full bg-[#1c6576] relative overflow-hidden py-24 px-6 md:px-12 lg:px-24">
        {/* Background decorative gradient */}
        <div className="absolute inset-0 bg-gradient-to-br from-[#1c6576] to-[#145562] z-0"></div>
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-8">
            <span className="inline-block py-1.5 px-4 rounded-full bg-white/10 border border-white/20 text-white/90 text-sm font-bold tracking-widest uppercase backdrop-blur-sm">
              Repositorio
            </span>
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 lg:px-24 -mt-8 relative z-20 pb-24">
        ...
        {session?.user && (
          <div className="bg-white rounded-3xl p-8 mb-12 shadow-[0_8px_30px_rgb(0,0,0,0.06)] border border-[#d8ddd9]">
            <div className="mb-6 border-b border-[#d8ddd9] pb-4">
              <h2 className="text-2xl font-heading font-bold text-[#28282b] mb-2">Subir nuevo paper</h2>
              <p className="text-[#5a5a60]">Formatos permitidos: PDF. Tamaño máximo recomendado: 20 MB.</p>
            </div>
            <UploadForm />
          </div>
        )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <aside className="lg:col-span-1">
            <div className="bg-white rounded-3xl p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-[#d8ddd9] sticky top-28">
              <h2 className="text-xl font-bold text-[#28282b] mb-6 flex items-center">
                <svg className="w-5 h-5 mr-2 text-[#1c6576]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V17l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.293A1 1 0 013 6.586V4z" /></svg>
                Filtrar Resultados
              </h2>

              <form method="get" className="space-y-5">
                <div className="space-y-2">
                  <label htmlFor="q" className="text-sm font-bold text-[#3a3a40] block">Búsqueda general</label>
                  <input
                    id="q"
                    name="q"
                    type="search"
                ...
                <div className="space-y-2">
                  <label htmlFor="author" className="text-sm font-bold text-[#3a3a40] block">Autor principal</label>
                  <div className="relative">
                    <select
                      id="author"
                      name="author"
                      defaultValue={selectedAuthor}
                      className="w-full bg-[#f5f7f5] border border-[#d0d5d1] rounded-xl px-4 py-2.5 appearance-none focus:outline-none focus:ring-2 focus:ring-[#1c6576]/30 focus:border-[#1c6576] transition-all text-[#28282b]"
                    >
                    ...
                    <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center px-4 text-[#5a5a60]">
                ...
                <div className="space-y-2">
                  <label htmlFor="journal" className="text-sm font-bold text-[#3a3a40] block">Revista Científica</label>
                  <div className="relative">
                    <select
                      id="journal"
                      name="journal"
                ...
                <div className="space-y-2">
                  <label htmlFor="year" className="text-sm font-bold text-[#3a3a40] block">Año de publicación</label>
                  <div className="relative">
                    <select
                      id="year"
                      name="year"
                ...
                <button type="submit" className="w-full bg-[#1c6576] hover:bg-[#145562] text-white font-bold py-3 px-4 rounded-xl transition-all shadow-md hover:shadow-lg transform hover:-translate-y-0.5 mt-4">
                  Aplicar Filtros
                </button>
              </form>
            </div>
          ...
          <div className="lg:col-span-3">
            ...
            {filteredPapers.length === 0 ? (
              <div className="bg-white rounded-3xl p-12 text-center shadow-sm border border-[#d8ddd9]">
                <div className="w-20 h-20 bg-[#f5f7f5] rounded-full flex items-center justify-center mx-auto mb-6">
                  <svg className="w-10 h-10 text-[#5a5a60]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9.172 16.172a4 4 0 015.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                </div>
                <h3 className="text-2xl font-bold text-[#28282b] mb-2">No se encontraron publicaciones</h3>
                <p className="text-[#5a5a60] text-lg">Intenta ajustar los filtros de búsqueda o prueba con otros términos.</p>
              </div>
            ) : (
              <div className="space-y-6">
                {filteredPapers.map((paper) => {
                  ...
                  return (
                    <article key={paper.id} className="bg-white rounded-3xl p-6 md:p-8 shadow-[0_4px_20px_rgb(0,0,0,0.03)] hover:shadow-[0_12px_30px_rgb(0,0,0,0.08)] border border-[#d8ddd9] transition-all duration-300 flex flex-col sm:flex-row gap-6 md:gap-8 group">

                      <div className="flex-1">
                        <div className="flex flex-wrap items-center gap-3 mb-4">
                          <span className="bg-[#f5f7f5] text-[#1c6576] font-bold text-sm px-3 py-1 rounded-full border border-[#d0d5d1]">
                            {publicationYear}
                          </span>
                          <span className="text-[#5a5a60] text-sm flex items-center">
                        ...
                        <h3 className="text-xl md:text-2xl font-bold text-[#28282b] mb-3 leading-snug group-hover:text-[#1c6576] transition-colors">
                          {paper.title}
                        </h3>

                        <p className="text-[#3a3a40] font-medium mb-4 flex items-start">
                          <svg className="w-5 h-5 mr-2 mt-0.5 text-[#a0a0a8] shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" /></svg>
                          {paper.authors}
                        </p>

                        {paper.summary && (
                          <p className="text-[#5a5a60] leading-relaxed line-clamp-3 mb-6">
                            {paper.summary}
                          </p>
                        )}
                      </div>
                      ...
                      <div className="sm:w-48 flex sm:flex-col items-center sm:items-end justify-between sm:justify-start pt-2 shrink-0 border-t sm:border-t-0 sm:border-l border-[#d8ddd9] sm:pl-6 gap-3">
                        <Link
                          ...
                          target="_blank"
                          className="w-full text-center bg-[#1c6576] hover:bg-[#145562] text-white px-5 py-3 rounded-xl font-bold transition-all shadow-md hover:shadow-lg flex items-center justify-center gap-2 group-hover:scale-105"
                        >
