...
export async function POST(request: Request) {
  ...
  if (!user || user.role !== "ADMIN") {
    return NextResponse.json(
      { error: "Solo administradores pueden publicar entradas de vinculacion." },
      { status: 403 },
    );
  }
  ...
  if (image instanceof File && image.size > 0) {
    ...
    const {
      data: { publicUrl },
    } = supabaseAdmin.storage.from("outreach").getPublicUrl(finalFileName);
  ...
  const entry = await prisma.outreach.create({
    data: {
      title,
      body,
      category,
